...
* In the Quantities sub-folder, add a new member-less struct implementing the IQuantity interface:
...
* Add the units supported by this quantity as static readonly public fields of the new struct. A standard unit should always be included, instantiated with the single argument (unit symbol) constructor. The secondary units are instantiated by defining a conversion factor or conversion functions with regards to the standard unit:
...
* Define a static readonly private field describing the SI unit dimensions of the quantity as a combination of the SI base unit dimensions:
   ...
   {
     ...
     private static readonly QuantityDimensions _dimensions = 2 * Length.Base + Mass.Base - 2 * Time.Base;
   }

* Implement the properties of the IQuantity interface, returning the defined dimensions and standard unit, respectively, of the new quantity:
